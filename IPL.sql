...
SELECT bidder_id, count(*)/(select count(*) from  ipl_bidding_details) *100 as Percentage  FROM ipl_bidding_details
...
select * from ipl_match_schedule;
select * from ipl_stadium;
select a.STADIUM_NAME , a.CITY ,  count(ms.stadium_id) as no_of_matches 
from ipl_stadium a  left join ipl_match_schedule ms
on a.STADIUM_ID = ms.STADIUM_ID
group by  a.STADIUM_NAME , a.CITY;

-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
/* select * from ipl_stadium;
select * from ipl_match;
select * from ipl_match_schedule where stadium_id = 7 ;
select  stadium_id , count(match_id) total_match from ipl_match_schedule  group by stadium_id ;
select TOURNMT_ID,  stadium_id , count(match_id) from ipl_match_schedule group by stadium_id ,TOURNMT_ID;
select count(im.match_id) as toss_factor , stadium_id 
from ipl_match im join ipl_match_schedule ims
on im.match_id = ims.match_id
where status = 'completed' and (TOSS_WINNER = MATCH_WINNER)
group by  stadium_id
order by toss_factor desc ;
*/

with toss_decides_match_winner as (
select count(im.match_id) as toss_factor , stadium_id 
from ipl_match im join ipl_match_schedule ims
on im.match_id = ims.match_id
where status = 'completed' and (TOSS_WINNER = MATCH_WINNER)
...
select a.stadium_id ,((toss_factor)/count(match_id)) *100  as winning_percentage
from toss_decides_match_winner a join ipl_match_schedule b
...
select bidder_id , no_of_bids from ipl_bidder_points;
select team_name , bid_team ,count( bidder_id) total_bids
...
select win_details,match_winner, case 
when match_winner = 1 THEN team_id1
when match_winner=2 then team_id2
else null
end as winner_team_id from ipl_match;

/*
select im.match_id , it.team_id , im.win_details , it.team_name
from ipl_match im join ipl_team it
on it.team_id = (team_id1 , team_id2) ;

select m1.TEAM_ID1,m1.TEAM_ID2,m1.MATCH_WINNER , m2.MATCH_WINNER
from ipl_match m1 join ipl_match m2
on m1.MATCH_WINNER = m2.MATCH_WINNER
where m1.MATCH_WINNER in (m1.TEAM_ID1,m1.TEAM_ID2);
*/


-- 6.	Display total matches played, total matches won and total matches lost by the team along with its team name.

with team_performance_details as (
select  team_id , sum(matches_played) as total_matches_palyed , sum(matches_won) as total_winnings , sum(MATCHES_LOST) as total_lost 
from ipl_team_standings
group by team_id)

select it.team_name , tpd.* 
from ipl_team it join team_performance_details tpd
on it.team_id = tpd.team_id;

-- 7.	Display the bowlers for the Mumbai Indians team.
with mi_bowlers as (
select * from ipl_team_players
where PLAYER_ROLE = 'Bowler' and team_id = 5)
select ip.player_name  , mb.*
from ipl_player ip join mi_bowlers mb
on ip.player_id = mb.player_id;
...
select team_id , count(player_role) as all_rounders 
...
select * from ipl_bidding_details;
select * from ipl_team;
select distinct BID_STATUS from ipl_bidding_details;

select bid_status,year(bid_date),sum(total_points) from ipl_bidder_points ibp 
join ipl_bidding_details ibd on ibp.bidder_id=ibd.bidder_id
join ipl_match_schedule ims on ims.schedule_id=ibd.schedule_id
join ipl_stadium ist on ist.stadium_id=ims.stadium_id
join ipl_match im on im.match_id=ims.match_id
where (bid_team=(select team_id from ipl_team where remarks="CSK")) AND (stadium_name = "M. Chinnaswamy Stadium")and (im.win_details like "%CSK won%")
...
select * from ipl_team;
select * from ipl_team_players;
select SUBSTRING_INDEX(PERFORMANCE_DTLS, 'Wkt-', -1) from ipl_player;

with bowler_allrounder as (
select team_name , player_id , player_role from ipl_team_players 
join ipl_team 
using(team_id) where player_role in ('bowler','all-rounder'))
,
player_wickets_ranking as (
select player_id , player_name ,lpad(SUBSTRING_INDEX(SUBSTRING_INDEX(PERFORMANCE_DTLS, 'Wkt-', -1), ' ', 1),2,0) as wickets from ipl_player order by wickets desc)
,
over_all as (select a.* , player_name ,wickets  , row_number() over (order by wickets desc) as rk
from bowler_allrounder a join player_wickets_ranking b
using(player_id) )
...
with mini_1 as(
with mini as
(select ibd.bidder_id,im.match_id, bid_team ,(select case when toss_winner=1 then team_id1 when toss_winner=2 then team_id2 else null end  ) as toss_winner_tid from ipl_bidder_points ibp 
join ipl_bidding_details ibd on ibp.bidder_id=ibd.bidder_id
join ipl_match_schedule ims on ims.schedule_id=ibd.schedule_id
join ipl_match im on im.match_id=ims.match_id)
select bidder_id,count(match_id)  as toss_wins from mini
where bid_team=toss_winner_tid
group by bidder_id)
select *,(select no_of_bids from ipl_bidder_points ibp where ibp.bidder_id=mini_1.bidder_id ) as total_bids,toss_wins/(select no_of_bids from ipl_bidder_points ibp where ibp.bidder_id=mini_1.bidder_id )*100 as percent from mini_1
...
-- Output columns should be like the below:
 ...
 select max(duration_days ) as max_duration, min(duration_days) as min_duration from duration;
 
  with mini as
 (select tournmt_id,tournmt_name,from_date,to_date,datediff(to_date,from_date) as duration,rank()over(order by (datediff(to_date,from_date))) as ranks from ipl_tournament)
 select * from mini
 where ranks<2
 union 
 (with mini_1 as
 (select tournmt_id,tournmt_name,from_date,to_date,datediff(to_date,from_date) as duration,rank()over(order by (datediff(to_date,from_date)) desc) as ranks from ipl_tournament)
 select * from mini_1
 where ranks<2);
...
select * from ipl_bidding_details;
select * from ipl_bidder_details;
/* select distinct a.BIDDER_ID, BIDDER_NAME, sum(TOTAL_POINTS) as total_pts , year(BID_DATE) as bid_year , month(BID_DATE) as bid_month
from ipl_bidder_details a join ipl_bidding_details b 
using (BIDDER_ID)
join ipl_bidder_points c
using(BIDDER_ID)
where BID_DATE like '%2017%'
group by a.BIDDER_ID, BIDDER_NAME, (BID_DATE)
order by total_pts desc , bid_month asc ;
select year(BID_DATE) as bid_year , monthname(BID_DATE) from ipl_bidding_details where bid_date like '%2017%';
*/

select ibd.bidder_id,ibdd.bidder_name, year(bid_date), month(bid_date), sum(total_points) as total_points from ipl_bidder_points ibp 
join ipl_bidding_details ibd on ibd.bidder_id=ibp.bidder_id 
join ipl_bidder_details ibdd on ibdd.bidder_id=ibd.bidder_id
where year(bid_date)=2017
group by month(bid_date),ibd.bidder_id,ibdd.bidder_name, year(bid_date)
order by total_points desc, month(bid_date) asc;

-- 14.	Write a query for the above question using sub queries by having the same constraints as the above question.

with mini as(select bidder_id,year(bid_date)as year, month(bid_date)as month,sum((select sum(total_points) from ipl_bidder_points ibp where ibp.bidder_id=ibd.bidder_id group by ibp.bidder_id)) as total_points 
from ipl_bidding_details ibd where year(bid_date)=2017 
group by month(bid_date), bidder_id,year(bid_date)
order by total_points desc, month(bid_date))
select bidder_id,(select bidder_name from ipl_bidder_details ibd where ibd.bidder_id=mini.bidder_id) as bidder_name, year, month, total_points  from mini ;
...
with mini as 
(select ibp.bidder_id,bidder_name, total_points,dense_rank() over(order by total_points desc) as ranks_1 from ipl_bidder_points ibp join ipl_bidder_details ibd on ibd.bidder_id=ibp.bidder_id where  tournmt_id=2018)
select * from mini
where ranks_1<4 
union 
(with mini_1 as 
(select ibp.bidder_id,bidder_name,total_points,dense_rank() over(order by total_points ) as ranks_2 from ipl_bidder_points ibp join ipl_bidder_details ibd on ibd.bidder_id=ibp.bidder_id where  tournmt_id=2018)
select * from mini_1
where ranks_2<4 );